...
func dataSourceSensitive() *schema.Resource {
	return &schema.Resource{
		Description: "The `external_sensitive` data source allows an external program implementing a specific protocol " +
		...
		Schema: map[string]*schema.Schema{
			...
			"query": {
				Description: "A map of string values to pass to the external program as the query " +
					...
					"Its value is marked as Sensitive and will be masked in the UI.",
				Type:      schema.TypeMap,
				Optional:  true,
				Sensitive: true,
				Elem: &schema.Schema{
